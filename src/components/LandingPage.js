...
const LandingPage = (props) => {
  ...
  async function sendToPost() {
    validateFields(async (errors, values) => {
      if (!errors) {
        ...
        axios
          .post(
            `${BASE_URL}admin/products/register/${userId()}`,
            formData,
            {
              headers: {
  ...
  const getData = async () => {
    axios.get(`${BASE_URL}admin/products/list/all`).then((response) => {
  ...
  useEffect(() => {
    ...
    if (checkLogin() == false) {
      ...
      axios
        .get(`${BASE_URL}admin/users/detail/${userId()}`)
        .then((response) => {
          ...
          {list.map((item) => (
            <Col md={6}>
              ...
              >
                ...
                >
                  {item.owner_product == userId() && (
                    <>
                      <p> * Este é um produto seu *</p>
                      <Button
                        shape="round"
                        onClick={() => {
